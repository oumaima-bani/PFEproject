class OffresController < ApplicationController
  before_action :authorize_request, except: %i[missiondata index get_Freelance_offre_by_Frelancer get_Emploi_offre_by_Jobseeker get_Stage_offre_by_Intern getoffrebydomain getoffrebydomain2 getoffrebydomain3 getoffrebybudget2 getoffrebybudget3 getoffrebybudget ]
 ...
 #**********Ended Offre(JOB + Internship + Freelance) [Admin]**********#
    ...
    def getalllocations
      render json: Offre.distinct.pluck(:location)
    end
    def getoffrebydomain
      if params[:domain_id].present?
        @offre = Offre.includes(:user)
                      .where(type_offre: "Emploi")
                      .includes(offre_categories: { souscategorie: { categorie: :domain } })
                      .where(domains: { id: params[:domain_id] })
      end
      render json: @offre, include: { user: { methods: [:user_image_url] },
                                      offre_categories: { include: { souscategorie: { include: { categorie: { include: :domain } } } } } }
    end
    def getoffrebydomain2
      if params[:domain_id].present?
        @offre = Offre.includes(:user)
                      .where(type_offre: "Stage")
                      .includes(offre_categories: { souscategorie: { categorie: :domain } })
    ...
    def getoffrebybudget
      @offre = Offre.includes(:user)
        .where(type_offre: "Emploi")
        .where('salaire BETWEEN ? AND ?', params[:min_salary], params[:max_salary])
        .order('salaire DESC')
        .includes(offre_categories: { souscategorie: { categorie: :domain } })
      render json: @offre, include: {
