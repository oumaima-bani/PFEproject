class CandidaturesController < ApplicationController
 ...
 #************************* custom functionlity ***************************#
    ...
    def get_active_offre_by_freelance
        @candidature = Candidature.where(user_id: params[:user_id]).where(etat: "Accepted").includes(:offre).where(offres: { is_completed: false })
        render json: @candidature, include: {
          offre: {},
          user: { methods: [:user_image_url] }
        }, methods: [:formatted_created_at, :formatted_created_at2]
    end
    def get_ended_offre_by_freelance
        @candidature = Candidature.where(user_id: params[:user_id]).where(etat: "Accepted").includes(:offre).where(offres: { is_completed: true })
        render json: @candidature, include: {
          offre: {},
          user: { methods: [:user_image_url] }
        }, methods: [:formatted_created_at, :formatted_created_at2]
